...
test('one player races three server-controlled robots and reconnects', async ({ page, request }) => {
  ...
  const rejectedJoin = await request.post(`/api/rooms/${code}/join`, { data: { displayName: 'Intruder', robotId: 'twonky' } });
  ...
  const before = await page.evaluate(() => (window as typeof window & { __VIBE_MATCH__?: { public: { revision: number } } }).__VIBE_MATCH__?.public.revision ?? 0);
  ...
  await expect.poll(async () => page.evaluate((startingRevision) => {
    const match = (window as typeof window & { __VIBE_MATCH__?: { public: { revision: number; phase: string; timerDeadline?: number }; events: Array<{ type: string; seatId?: string }> } }).__VIBE_MATCH__;
    return Boolean(match && match.public.revision >= startingRevision + 4 && ['programming', 'complete'].includes(match.public.phase) && !match.public.timerDeadline && match.events.filter((event) => event.type === 'program-ready' && event.seatId?.startsWith('cpu-')).length === 3);
  }, before)).toBe(true);
...
test('room creation rejects an unknown match mode', async ({ request }) => {
  const response = await request.post('/api/rooms', { data: { displayName: 'Ada', robotId: 'hammer-bot', mode: 'arcade' } });
