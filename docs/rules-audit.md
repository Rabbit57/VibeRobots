...
Archive reentry now pauses cleanup for each human to choose a facing. When the archive is occupied, the player chooses a legal orthogonal or diagonal square and a facing that does not sight another robot within three squares; if the adjacent band has no legal choice, the search expands outward as directed by the FAQ. CPU robots choose automatically. Destruction and playback carry the exact remaining life count so a death animation cannot remove another heart. Conveyor bends announce the resulting heading change in the turn timeline.
