...
export function HomePanel(props: { name: string; setName: (value: string) => void; robotId: string; setRobotId: (value: string) => void; roomCode: string; setRoomCode: (value: string) => void; createRoom: () => void; joinRoom: () => void; busy: boolean; error: string }) {
  return <aside className="cozy-panel launch-panel">
    <p className="kicker">ONLINE DIORAMA RACING · 2–8 PLAYERS</p>
    ...
    <div className="launch-actions">
      <button className="primary" type="button" disabled={props.busy} onClick={props.createRoom}>CREATE A COZY ROOM <span>→</span></button>
    ...
    <div className="feature-row"><span>Private rooms</span><span>Server fair-play</span><span>No account</span></div>
  </aside>;
}
...
export function LobbyPanel({ view, roomCode, isHost, courseId, setCourseId, fourLives, setFourLives, start, error }: { view: PrivateMatchView; roomCode: string; isHost: boolean; courseId: string; setCourseId: (value: string) => void; fourLives: boolean; setFourLives: (value: boolean) => void; start: () => void; error: string }) {
  ...
  return <aside className="cozy-panel lobby-panel">
    <div className="lobby-head"><div><p className="kicker">PRIVATE WORKSHOP</p><h2>{roomCode}</h2></div><button type="button" onClick={copyInvite}>COPY INVITE</button></div>
    <div className="seat-grid">{view.public.robots.map((robot) => { const identity = ROBOT_BY_ID.get(robot.robotId)!; return <div className="seat" key={robot.seatId}><img src={identity.portraitUrl} alt=""/><div><strong>{robot.displayName}</strong><small>{identity.name}{robot.seatId === view.public.hostSeatId ? ' · HOST' : ''}</small></div><i className={robot.connected ? 'online' : ''}/></div>; })}{Array.from({ length: Math.max(0, 2 - view.public.robots.length) }, (_, index) => <div className="seat empty" key={index}>A tiny robot is still on its way…</div>)}</div>
