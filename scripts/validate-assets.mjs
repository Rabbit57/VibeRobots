...
const robotIds = ['hammer-bot', 'hulk-x90', 'spin-bot', 'squash-bot', 'trundle-bot', 'twitch', 'twonky', 'zoom-bot'];
...
for (const id of robotIds) {
  ...
  const positionAccessors = (json.accessors ?? []).filter((accessor) => accessor.type === 'VEC3' && accessor.min && accessor.max);
...
for (const name of ['tile_base', 'wall', 'conveyor', 'express_conveyor', 'gear', 'pusher', 'laser', 'repair', 'checkpoint', 'dock', 'pit_rim']) {
...
for (const image of ['vibe-robots-key-art.webp', 'course-risky-exchange.webp', 'course-dizzy-dash.webp', 'course-against-the-grain.webp']) {
...
console.log(`Validated 9 GLBs and 16 runtime images (${(modelBytes / 1024).toFixed(0)} KB of models).`);
